		...
		/* Enter your code here */
			for (int i = 0; i < MAX_ROW; i++) {
				for (int j = 0; j < MAX_COL; j++) {
					...
					if (currentTile != goalTile && currentTile !=0) {
						output++;
					}
				}
			}
	...
	public int computeH2(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				...
				int currentPos = currentState.getTile(i, j);
				int[] goal = goalState.getLocation(currentPos);
				if(currentPos != 0) {
					int row = Math.abs(i - goal[0]);
					int col = Math.abs(j - goal[1]);
					result += row + col;
				}
			}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			if (row >= 0) {// Tile stands inside the map
				...
				result.updateTile(newRow, col, 0);
			...
			if (col >= 0) {
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, newCol, 0);
				result.updateTile(row, col, tmp);
				result.setH(computeH2(result));
				return result;
			}
